...
router.get('/', async (req, res) => {
  ...
  try {
    if (boundingBox) {
      const resolvedArea = await resolveCityFromBoundingBox(boundingBox);
      const labelledCity = city || resolvedArea.city || 'Área seleccionada';
      const areaLabel =
        resolvedArea.displayName || resolvedArea.city || 'Área delimitada en el mapa';
    ...
    if (!city) {
      return res
        .status(400)
        .json({ error: 'Indica un municipio o dibuja un área para buscar puntos.' });
    }
